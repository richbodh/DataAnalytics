...
with conv_enq as(
select distinct enquiry_id, enq.date, enq.user_id, txn_id from enquiries enq
inner join (select distinct t.user_id, txn_id, min(e.date) as edate from txns t left join enquiries e on t.user_id = e.user_id
where e.date <= t.date and datediff(day, e.date, t.date)<=30 
group by t.user_id, txn_id, t.date
order by t.user_id) earl_enq on enq.user_id = earl_enq.user_id
where enq.date = earl_enq.edate)

insert into enq_cnv_map
select * from conv_enq 
UNION ALL
select distinct enquiry_id, e.date, e.user_id, '' as txn_id from enquiries e inner join txns t on e.user_id = t.user_id
where e.enquiry_id not in (select distinct enquiry_id from conv_enq) and (e.date > t.date or datediff(day, e.date, t.date)>30) 
group by enquiry_id, e.date, e.user_id;
 
select distinct enquiry_id, date, user_id, ARRAY_AGG(txn_ids) from enq_cnv_map
